...
public class WorldFrame<T> extends JFrame
{
    ...
    public WorldFrame(World<T> world)
    {
        ...
        display = new GridPanel(displayMap, resources, world.getType());
        ...
        //control.controlPanel().setBackground(CustomColor.green());
        control.controlPanel().setBackground(Color.BLACK);
        content.add(control.controlPanel(), BorderLayout.SOUTH);
        //content.setBackground(CustomColor.green());
	...
	public void repaint()
    {
    	int credits = this.getWorld().getPlayer().getCredits();
    	int kills = this.getWorld().getPlayer().getDeaths();
        //String message = getWorld().getMessage();
        //if (message == null)
        //String message = resources.getString("message.pvz");
    	String message = "";
    	if(worldType.equals("Display"))
    	message = "                     Welcome to the Encyclopedia of Plants and Zombiesss";
    	else
    	message = "               Protect yer house from the Zombies or THEY'LL EAT YER BRAINSSSS!\n" + "                "+ "Time Elapsed: " + Player.getTime() +"        Zombies Killed: " + kills + "       " + " SunCredits: " + credits;
        messageArea.setText(message);
        Font font2 = new Font("MonoSpaced",Font.BOLD + Font.ITALIC,25);
